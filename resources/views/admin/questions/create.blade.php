@extends('layouts.admin', ['title' => 'Dodaj nowe pytanie'])
...
@section('content')
    <div class="space-y-6">
        <div>
            ...
            <h1 class="text-3xl font-semibold text-white">Dodaj nowe pytanie</h1>
            <p class="text-slate-400 mt-1">Quiz: {{ $quiz->title }}</p>
        </div>

        <form action="{{ route('admin.quizzes.questions.store', $quiz) }}" method="POST" class="space-y-6" id="questionForm">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div id="openQuestionFields" style="display: {{ old('is_open') ? 'block' : 'none' }};">
                    <div>
                        ...
                        >{{ old('correct_answer_text') }}</textarea>
                ...
                <div id="closedQuestionFields" style="display: {{ old('is_open') ? 'none' : 'block' }};">
                    <div>
                        ...
                        <div id="answersContainer" class="space-y-3">
                            ...
                            @else
                                <div class="answer-item flex gap-3 items-start">
                                    ...
                                    >
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="answers[0][is_correct]" value="1" class="h-5 w-5 accent-indigo-500">
                                        <span class="text-sm text-slate-300">Poprawna</span>
                                    </label>
                                    <button type="button" onclick="removeAnswer(this)" class="p-2 text-rose-400 hover:text-rose-300">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div class="answer-item flex gap-3 items-start">
                                    <input
                                        type="text"
                                        name="answers[1][answer_text]"
                                        placeholder="Treść odpowiedzi..."
                                        class="flex-1 rounded-2xl border border-white/10 bg-white/5 p-3 text-base text-white placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none"
                                    >
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="answers[1][is_correct]" value="1" class="h-5 w-5 accent-indigo-500">
                                        <span class="text-sm text-slate-300">Poprawna</span>
                                    </label>
                                    <button type="button" onclick="removeAnswer(this)" class="p-2 text-rose-400 hover:text-rose-300">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
